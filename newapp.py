...
UUID = os.environ.get('UUID', 'b7d62986-7307-4f98-a09f-3dd73f642e6e')  # UUID,如使用哪吒v1,在不同的平台部署需要修改,否则会覆盖
NEZHA_SERVER = os.environ.get('NEZHA_SERVER', 'nz.xinci.pp.ua:8110')      # 哪吒面板域名或ip, v1格式: nezha.xxx.com:8008, v0格式: nezha.xxx.com
NEZHA_PORT = os.environ.get('NEZHA_PORT', '')          # v1哪吒请留空, v0哪吒的agent通信端口,自动匹配tls
NEZHA_KEY = os.environ.get('NEZHA_KEY', 'zfiDIKTVptNNhAWORGrdNyAA6UgynKJF')            # v1哪吒的NZ_CLIENT_SECRET或v0哪吒agent密钥
ARGO_DOMAIN = os.environ.get('ARGO_DOMAIN', 'appwriteus.xcl.pp.ua')        # Argo固定隧道域名,留空即使用临时隧道
ARGO_AUTH = os.environ.get('ARGO_AUTH', 'eyJhIjoiYzJlZDAyMmQ3YTA0NDI0YWJhMGRkNDI0ZGQ3NzlmMGQiLCJ0IjoiNzJmNzlmN2UtZmVlYS00MWQ1LTk0YmQtYmZjYmZlOTM3NjI3IiwicyI6Ik56VTFOMkppTlRRdE16Z3pNQzAwTkRBM0xUbGlaRFF0TmpRelltRTRNVGMxTUdKaiJ9')            # Argo固定隧道密钥,留空即使用临时隧道
ARGO_PORT = int(os.environ.get('ARGO_PORT', '10086'))   # Argo端口,使用固定隧道token需在cloudflare后台设置端口和这里一致
CFIP = os.environ.get('CFIP', 'v.xcl.pp.ua')       # 优选ip或优选域名
CFPORT = int(os.environ.get('CFPORT', '443'))          # 优选ip或优选域名对应端口
NAME = os.environ.get('NAME', 'appwriteus')                   # 节点名称
...
async def generate_links(argo_domain):
    ...
    try:
        ...
        if result.returncode == 0 and result.stdout:
            ...
            isp = meta_json.get("isp", "UnknownISP")
            country = meta_json.get("country", "XX")
            ISP = f"{isp}-{country}".replace(" ", "_")
    except Exception as e:
